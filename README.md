...
- **Fight discussions** on every matchup's Discussion tab: Reddit-style threads three levels deep, votes, permalinks, reports, blocks, and a moderation queue at `/admin`. Moderation is light-touch — no word is banned; spam, floods, shouting, personal info and self-harm incitement are refused with a reason, and comments reported by three established accounts are held for review. Profiles gain a Comments tab, hidden from others until the owner chooses to show it. See [policy, limits and API](docs/discussions.md).
