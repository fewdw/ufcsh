...
export type Bet = { id: string; placedAt: number; stake: number; price: string; state: BetState; net: number; payout: number; legs: BetLeg[] };
...
export async function placeBet(token: string, stake: number, legs: { outcome: Outcome; price: string }[]): Promise<Bet> {
