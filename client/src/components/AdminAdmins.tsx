    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <input
          ...
          autoComplete="off"
          className="w-64 rounded-md border border-zinc-200 bg-white px-2.5 py-1.5 text-sm outline-none focus:border-zinc-400"
        />
        <button
          type="submit"
      ...
      {failed ? <p className="rounded-md bg-red-50 px-3 py-2 text-xs text-red-700">{failed}</p> : null}
            ...
            <li key={admin.email} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
              ...
              {admin.removable ? (
                <button
                  ...
                  className="rounded-md border border-zinc-200 px-2.5 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-50 disabled:opacity-50"
                >
                  Remove
                </button>
              ) : (
