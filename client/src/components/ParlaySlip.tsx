...
import { ChevronDown, Ticket, Trash2, X } from "lucide-react";
import { useParlay, parlayPayout, formatCombinedPrice } from "../parlay";
            ...
            <label className="flex items-center justify-between gap-2 text-xs font-medium text-zinc-500">
              ...
              <span className="flex items-center gap-1 rounded-lg border border-zinc-200 bg-white px-2 py-1">
                ...
                <input
                  ...
                  value={stakeText}
                  onChange={(event) => {
                    const cleaned = sanitizeStake(event.target.value);
                    setStakeText(cleaned);
                    const parsed = Number.parseFloat(cleaned);
                    if (Number.isFinite(parsed)) setStake(parsed);
                  }}
