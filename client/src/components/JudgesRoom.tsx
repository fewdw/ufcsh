  ...
  return <section className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
    <header className="border-b border-zinc-100 px-4 py-2">
      ...
      {selection ? <div className="mt-1 flex items-center gap-2">
        ...
        <label className="relative ml-auto min-w-40 flex-1 sm:max-w-64"><Search className="pointer-events-none absolute left-2 top-1/2 h-3 w-3 -translate-y-1/2 text-zinc-400" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search fighters or events" aria-label="Search scorecard fights" className="w-full rounded-lg border border-zinc-200 py-1.5 pl-7 pr-2 text-[10px] outline-none placeholder:text-zinc-400 focus:border-zinc-500" /></label>
      </div> : null}
    </header>
  ...
  return <>
    ...
    <div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <ChartCard title="The cards they wrote" subtitle={`Every final score · ${compact(j.cards)} cards · click a score to see its fights`} legend={<Legend items={[{ label: "Share of cards", color: SERIES[0], shape: "line" }]} />}>
        <div className="border-b border-zinc-100 px-3 py-2"><label className="relative block"><Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-zinc-400" /><input type="search" value={scoreSearch} onChange={(event) => setScoreSearch(event.target.value)} placeholder="Find a score, e.g. 50–45" aria-label="Search final scores" className="w-full rounded-lg border border-zinc-200 py-2 pl-8 pr-3 text-[11px] outline-none placeholder:text-zinc-400 focus:border-zinc-500" /></label></div>
...
function JudgesView({ j, metric, setMetric, judges, judgeSearch, setJudgeSearch, selection, setSelection, selectedMetric }: { j: LabsJudgesResponse; metric: JudgeMetric; setMetric: (value: JudgeMetric) => void; judges: JudgeRow[]; judgeSearch: string; setJudgeSearch: (value: string) => void; selection: Selection | null; setSelection: (value: Selection) => void; selectedMetric: (typeof METRICS)[number] }) {
  return <ChartCard title="Judge tendencies" subtitle={`${selectedMetric.hint} Click a judge to see every fight they scored.`}>
    <div className="border-b border-zinc-100 px-3 py-2"><div className="flex flex-wrap gap-1">{METRICS.map((item) => <button key={item.key} type="button" aria-pressed={metric === item.key} onClick={() => setMetric(item.key)} className={`rounded-full border px-2.5 py-1 text-[9px] font-bold transition ${metric === item.key ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-500 hover:border-zinc-400"}`}>{item.label}</button>)}</div><label className="relative mt-2 block"><Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-zinc-400" /><input type="search" value={judgeSearch} onChange={(event) => setJudgeSearch(event.target.value)} placeholder={`Search all ${j.officials.length} judges`} aria-label="Search judges" className="w-full rounded-lg border border-zinc-200 py-2 pl-8 pr-3 text-[11px] outline-none placeholder:text-zinc-400 focus:border-zinc-500" /></label></div>
